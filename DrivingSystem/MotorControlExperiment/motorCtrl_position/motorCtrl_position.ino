...
ISR(TIMER2_COMPA_vect){
    ...
    static double P_err = 0;
    static double P_G = 0;
    static double P_In = 0;
    
    static int P_loopCounter = 0;
    while(motor1Counter > resolution){
    motor1Counter -= resolution;
    motor1Rot++;
    }
    ...
    if(P_loopCounter>= P_loop){
        P_loopCounter = 0;
        long motor1Location = motor1Pos + resolution * motor1Rot;
        
        P_err = motor1Pcmd - motor1Location;
        P_G = P_err * P_KP;
        P_In = P_In + 0.0005 * P_err * P_KI;
        
        P_G =  (P_G>5  ? 5 : (P_G< -5 ? -5 : P_G));
        P_In = (P_In>5 ? 5 : (P_In<-5 ? -5 : P_In));
        motor1Vcmd = P_G + P_In;
        motor1Vcmd = (motor1Vcmd>5 ? 5 : (motor1Vcmd<-5 ? -5 : motor1Vcmd));
    }
    P_loopCounter++;
}
