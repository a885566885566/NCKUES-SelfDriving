...
void control_init(volatile CONTROL *ctrl){
    ...
    PID_init(&(ctrl->pid_velocity), 0.015, 0.025, 0, CONF_CONTROL_CURRENT_DT, CONF_PWM_LIM, -1*CONF_PWM_LIM, 0.9);
...
ISR(TIMER2_COMPA_vect){
    ...
    if (motor.current_counter > CONF_CONTROL_CURRENT_COUNT){
        ...
        double now_current = read_current();
        double now_speed = encoder_speed_mt(&(motor.enc));
        Serial.print(motor.pid_velocity.cmd/10);
        ...
        Serial.print(',');
        Serial.print(now_current);
        Serial.print(',');
        Serial.println(encoder_speed_mt(&(motor.enc))/10);
